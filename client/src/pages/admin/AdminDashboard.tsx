...
// Food Form Component
function FoodForm({ food, onClose }: { food?: any | null; onClose: () => void }) {
  const [formData, setFormData] = useState({
    name: food?.name || '',
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: any) => {
      ...
      const processedData = {
        ...data,
        ingredients: data.ingredients ? data.ingredients.split(',').map((item: string) => item.trim()).filter(Boolean) : [],
        restaurants: data.restaurants ? data.restaurants.split(',').map((item: string) => item.trim()).filter(Boolean) : [],
      };
        ...
        <CardContent className="p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <Label htmlFor="restaurants">Popular Restaurants (comma separated)</Label>
                <Input
                  id="restaurants"
                  name="restaurants"
...
function FoodContent() {
  ...
  const { data: foodItems = [], isLoading, refetch } = useQuery<any[]>({
      ...
      <Card>
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-8 text-center text-gray-500">
              {searchTerm || categoryFilter !== 'all' ? 'No food items match your filters' : 'No food items found. Add your first item!'}
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full">
                ...
                <tbody>
                  {filteredFood.map((food: any) => (
                    <tr key={food._id} className="border-b border-border hover:bg-muted/50">
                      <td className="p-4">
                        <img
                          src={food.imageUrl}
                          alt={food.name}
                      ...
                      <td className="p-4">
                        <Badge variant={food.isVegetarian ? "default" : "secondary"}>
      ...
      <Card>
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-8 text-center text-gray-500">
              {searchTerm || categoryFilter !== 'all' ? 'No events match your filters' : 'No events found. Add your first event!'}
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="w-full">
...
function EventForm({ event, onClose }: { event?: any | null; onClose: () => void }) {
...
function GalleryContent() {
  ...
  if (showForm) {
    return <GalleryForm galleryItem={editingGallery} onClose={handleFormClose} />;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Button onClick={() => setShowForm(true)} data-testid="button-add-image">
          <Plus className="h-4 w-4 mr-2" />
          Add Image
        </Button>
      </div>
      ...
      <Card>
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="p-8 text-center text-gray-500">
              {searchTerm || categoryFilter !== 'all' ? 'No gallery items match your filters' : 'No gallery items found. Add your first image!'}
            </div>
          ) : (
            <div className="p-4">
...
function ContactContent() {
  ...
  if (selectedMessage) {
    return <ContactMessageView message={selectedMessage} onClose={() => setSelectedMessage(null)} onReply={handleReply} />;
  }
              ...
              <table className="w-full">
                ...
                <tbody>
                  {filteredMessages.map((message: any) => (
                    <tr key={message._id} className={`border-b border-border hover:bg-muted/50 ${!message.isRead ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
                      <td className="p-4">
                        <div className="flex gap-2">
                          {!message.isRead && (
...
function SettingsContent() {
  ...
  const { data: settingsData = [], isLoading, refetch } = useQuery<any[]>({
                ...
                {editingKey === config.key ? (
                  ...
                  <div className="p-3 bg-muted rounded-lg">
                    <p className="text-sm font-mono">
                      {settings[config.key] ? 
                        (config.type === 'password' ? '••••••••' : settings[config.key]) : 
...
function GalleryForm({ galleryItem, onClose }: { galleryItem?: any | null; onClose: () => void }) {
  const [formData, setFormData] = useState({
...
function ContactMessageView({ message, onClose, onReply }: { message: any; onClose: () => void; onReply: (reply: string) => void }) {
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <p>{new Date(message.createdAt).toLocaleDateString('en-US', {
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
                  hour: '2-digit',
